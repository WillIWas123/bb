...
type Options struct {
	...
	Threads		int
	Wordlists	string
	firstResps	map[string]*theeFP.Response
...
func (opt *Options) CopyOptions(req *http.Request) *Options{
	newOpt := &Options{Ctx: opt.Ctx, Client: opt.Client,Delay:opt.Delay,Req:req, MaxHeaderLength:opt.MaxHeaderLength,RndStrLength:opt.RndStrLength,Verbose:opt.Verbose,Threads:1,Wordlists:opt.Wordlists,prefix:opt.prefix,firstResps:map[string]*theeFP.Response{},inProgress:map[string]bool{},ext:opt.ext,extext:opt.extext,mux:opt.mux, Recursion:opt.Recursion+1}
	return newOpt
...
func ParseFlags() *Options {
	...
	opt := &Options{Ctx: ctx, Client: client, Req: req, Delay: delay, MaxHeaderLength: mhl, Regex: regex, Verbose: verbose, RndStrLength: rndStrLength, Threads: threads, Wordlists: wordlists, firstResps: make(map[string]*theeFP.Response), inProgress: make(map[string]bool), mux: &sync.Mutex{}, ext: parseExts(ext), extext: parseExts(extext), MaxRecursion: maxRecursion}
	return opt
